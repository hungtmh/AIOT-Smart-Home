...
function mergeDeviceState(states, nextState) {
  const currentState = states.find(
    (state) => state.deviceId === nextState.deviceId,
  );
  if (!currentState) return [...states, nextState];

  const desiredIsNewer =
    timestamp(nextState.updatedAt) >= timestamp(currentState.updatedAt);
  const reportedIsNewer =
    timestamp(nextState.reportedAt) >= timestamp(currentState.reportedAt);
  const mergedState = {
    ...currentState,
    ...nextState,
    desiredState: desiredIsNewer
      ? nextState.desiredState
      : currentState.desiredState,
    updatedAt: desiredIsNewer ? nextState.updatedAt : currentState.updatedAt,
    reportedState: reportedIsNewer
      ? nextState.reportedState
      : currentState.reportedState,
    reportedAt: reportedIsNewer
      ? nextState.reportedAt
      : currentState.reportedAt,
  };

  return states.map((state) =>
    state.deviceId === nextState.deviceId ? mergedState : state,
  );
}
    ...
    function connect() {
      ...
      socket.addEventListener("message", (event) => {
        ...
        if (message.type === "AUTHENTICATED") {
          ...
          setDeviceStates((currentStates) =>
            message.data.reduce(
              (states, deviceState) => mergeDeviceState(states, deviceState),
              currentStates,
            ),
          );
          ...
          setError(
            message.message || "Realtime JWT was rejected by the backend.",
          );
        }
      });
